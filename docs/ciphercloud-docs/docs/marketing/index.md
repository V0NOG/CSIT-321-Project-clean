...
Cipher Cloud was built for CSIT321 capstone project the requirements were Privact Preserving Cloud Data Protection System and is positioned as a professional privacy layer for everyday cloud storage. It does not ask users to abandon Dropbox or Google Drive. Instead, it encrypts files in the browser before upload, stores only ciphertext in the connected cloud provider, and supports secure sharing through client side key rewrapping.
...
The main value of Cipher Cloud is that the user does not need to completely trust our server with their files. The file is encrypted before it leaves the browser, then uploaded to the connected cloud provider. We also added sharing, folders, audit events, and analytics so it feels more like a usable file manager rather than just a security demo.
...
We used an iterative approach because the project changed as we tested each feature. At first, the focus was mainly on encryption and upload/download. Once that worked, we added folders, sharing, connectors, analytics, and user interface improvements. Privacy was treated as a core requirement rather than something added at the end.
...
The interface used was TailAdmin which is a modern SaaS dashboard as users are aready familiar with that layout. The panels and features added were created to make the system feel professional and easier to understand and since encryption can be confusing the documentaiton and diagrams were included to explain what happens during upload, download and sharing.
