// frontend/src/pages/Security/TotpSetup.tsx
import { useEffect, useMemo, useState } from "react";
import { useLocation, useNavigate } from "react-router-dom";
import axios from "axios";
import PageMeta from "../../components/common/PageMeta";
import PageBreadcrumb from "../../components/common/PageBreadCrumb";
import Button from "../../components/ui/button/Button";
import Alert from "../../components/ui/alert/Alert";

type AlertVariant = "success" | "warning" | "error" | "info";

function authHeader() {
  const token = localStorage.getItem("token") || "";
  return token ? { Authorization: `Bearer ${token}` } : {};
}

export default function TotpSetup() {
  const [status, setStatus] = useState<"loading" | "enabled" | "disabled">("loading");

  // Setup flow state
  const [qr, setQr] = useState<string>("");
  const [tempSecret, setTempSecret] = useState<string>("");
  const [code, setCode] = useState<string>("");

  // Disable flow state
  const [disableCode, setDisableCode] = useState<string>("");
  const [showDisableForm, setShowDisableForm] = useState(false);

  const [busy, setBusy] = useState(false);

  const [toast, setToast] = useState<{
    show: boolean;
    variant: AlertVariant;
    title: string;
    message: string;
  }>({ show: false, variant: "info", title: "", message: "" });

  const loc = useLocation();
  const navigate = useNavigate();
  const onboarding = useMemo(
    () => new URLSearchParams(loc.search).get("onboarding") === "1",
    [loc.search]
  );

  function showAlert(variant: AlertVariant, title: string, message: string) {
    setToast({ show: true, variant, title, message });
  }

  async function loadStatus() {
    try {
      const res = await axios.get("http://localhost:5050/api/user/mfa", {
        withCredentials: true,
        headers: authHeader(),
      });
      setStatus(res.data.totpEnabled ? "enabled" : "disabled");
    } catch {
      setStatus("disabled");
    }
  }

  useEffect(() => {
    loadStatus();
  }, []);

  async function fetchSetup() {
    try {
      setBusy(true);
      const res = await axios.post(
        "http://localhost:5050/api/auth/totp/setup",
        {},
        { withCredentials: true, headers: authHeader() }
      );
      setQr(res.data.qr);
      setTempSecret(res.data.tempSecret);
      showAlert(
        "info",
        "QR Code Ready",
        "Scan the QR using Google Authenticator, 1Password, or Authy. Then enter the 6-digit code."
      );
    } catch (e: any) {
      showAlert(
        "error",
        "Failed to start 2FA setup",
        e?.response?.data?.error || "Please try again."
      );
    } finally {
      setBusy(false);
    }
  }

  async function enable() {
    if (!tempSecret || !code) {
      showAlert("warning", "Action Required", "Generate and scan the QR code, then enter the 6-digit code.");
      return;
    }
    try {
      setBusy(true);
      const res = await axios.post(
        "http://localhost:5050/api/auth/totp/enable",
        { tempSecret, code },
        { withCredentials: true, headers: authHeader() }
      );
      showAlert("success", "Two-Factor Enabled", res.data?.message || "Your account is now protected.");
      setStatus("enabled");
      setQr("");
      setTempSecret("");
      setCode("");
      if (onboarding) setTimeout(() => navigate("/", { replace: true }), 800);
    } catch (e: any) {
      showAlert("error", "Enable Failed", e?.response?.data?.error || "Invalid or expired code. Please try again.");
    } finally {
      setBusy(false);
    }
  }

  async function disable() {
    try {
      setBusy(true);
      await axios.post(
        "http://localhost:5050/api/auth/totp/disable",
        { code: disableCode || undefined },
        { withCredentials: true, headers: authHeader() }
      );
      showAlert("success", "Two-Factor Disabled", "Two-factor authentication has been turned off.");
      setStatus("disabled");
      setShowDisableForm(false);
      setDisableCode("");
    } catch (e: any) {
      showAlert("error", "Disable Failed", e?.response?.data?.error || "Invalid code. Please try again.");
    } finally {
      setBusy(false);
    }
  }

  function skip() {
    navigate("/", { replace: true });
  }

  if (status === "loading") {
    return (
      <>
        <PageMeta title="Two-Factor Authentication" description="Protect your account." />
        <PageBreadcrumb pageTitle="Security / Two-Factor Authentication" />
        <div className="flex items-center justify-center py-12 text-sm text-gray-500">Loading…</div>
      </>
    );
  }

  return (
    <>
      <PageMeta title="Two-Factor Authentication" description="Protect your account with a one-time code." />
      <PageBreadcrumb pageTitle="Security / Two-Factor Authentication" />

      {toast.show && (
        <div className="mb-4">
          <Alert variant={toast.variant} title={toast.title} message={toast.message} showLink={false} />
        </div>
      )}

      {/* Status badge */}
      <div className="mb-6 flex items-center gap-3">
        <span
          className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
            status === "enabled"
              ? "bg-success-50 text-success-600 dark:bg-success-500/10 dark:text-success-400"
              : "bg-gray-100 text-gray-500 dark:bg-white/[0.06] dark:text-gray-400"
          }`}
        >
          <span
            className={`inline-block w-1.5 h-1.5 rounded-full ${
              status === "enabled" ? "bg-success-500" : "bg-gray-400"
            }`}
          />
          {status === "enabled" ? "Enabled" : "Disabled"}
        </span>
      </div>

      {/* ── ENABLED STATE ── */}
      {status === "enabled" && (
        <div className="max-w-2xl rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                Two-Factor Authentication is Active
              </h3>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Your account requires a one-time code from your authenticator app on every sign-in.
              </p>
            </div>
          </div>

          <div className="mt-6">
            {!showDisableForm ? (
              <Button
                variant="outline"
                onClick={() => setShowDisableForm(true)}
                className="border-error-300 text-error-600 hover:bg-error-50 dark:border-error-700 dark:text-error-400"
              >
                Disable Two-Factor Authentication
              </Button>
            ) : (
              <div className="rounded-xl border border-error-200 bg-error-50 p-4 dark:border-error-800 dark:bg-error-500/10">
                <p className="mb-3 text-sm font-medium text-error-700 dark:text-error-400">
                  Enter the 6-digit code from your authenticator app to confirm disabling 2FA.
                </p>
                <input
                  className="w-48 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                  placeholder="123456"
                  inputMode="numeric"
                  maxLength={6}
                  value={disableCode}
                  onChange={(e) => setDisableCode(e.target.value.replace(/\D/g, ""))}
                />
                <div className="mt-3 flex gap-2">
                  <Button
                    size="sm"
                    onClick={disable}
                    disabled={busy || !disableCode}
                    className="bg-error-600 hover:bg-error-700 text-white"
                  >
                    {busy ? "Disabling…" : "Confirm Disable"}
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => {
                      setShowDisableForm(false);
                      setDisableCode("");
                    }}
                    disabled={busy}
                  >
                    Cancel
                  </Button>
                </div>
              </div>
            )}
          </div>
        </div>
      )}

      {/* ── DISABLED STATE: SETUP FLOW ── */}
      {status === "disabled" && (
        <div className="max-w-2xl rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                Set up Two-Factor Authentication
              </h3>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Scan the QR with Google Authenticator, 1Password, or Authy. Then enter the 6-digit code.
              </p>
            </div>
            {onboarding && (
              <Button size="sm" variant="ghost" onClick={skip} className="text-gray-500 hover:text-gray-700">
                Skip for now
              </Button>
            )}
          </div>

          <div className="mt-6 flex items-start gap-6">
            <div className="flex flex-col items-center">
              <div className="h-48 w-48 rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 flex items-center justify-center overflow-hidden">
                {qr ? (
                  <img src={qr} alt="QR" className="h-full w-full object-contain" />
                ) : (
                  <span className="text-xs text-gray-400">QR will appear here</span>
                )}
              </div>
              <Button className="mt-4" onClick={fetchSetup} disabled={busy || !!qr}>
                {qr ? "QR Generated" : "Generate QR"}
              </Button>
            </div>

            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                6-digit code
              </label>
              <input
                className="mt-2 w-56 rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                placeholder="123456"
                inputMode="numeric"
                maxLength={6}
                value={code}
                onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))}
                disabled={!qr}
              />
              <div className="mt-4 flex gap-3">
                <Button onClick={enable} disabled={busy || !code}>
                  Enable 2FA
                </Button>
                {!onboarding && (
                  <Button variant="outline" onClick={() => navigate(-1)} disabled={busy}>
                    Cancel
                  </Button>
                )}
              </div>
            </div>
          </div>
        </div>
      )}
    </>
  );
}
