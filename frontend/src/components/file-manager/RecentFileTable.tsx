...
const IconBtn = ({ title, onClick, children, disabled }: any) => (
  <button
    ...
    onClick={onClick}
    disabled={disabled}
    className={`p-2 rounded-md text-gray-500 hover:text-gray-800 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white/90 dark:hover:bg-white/5 ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
  >
    {children}
  </button>
);
...
const TrashSvg = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden>
    <path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2m1 0l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6m3 0v14m4-14v14m4-14v14"
      stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
const SaveSvg = () => (
...
export default function RecentFileTable() {
  ...
  function showAlert(kind: AlertState["kind"], title: string, message: string) {
    ...
    // auto-hide success/info after a short delay
        ...
        <div className="px-6">
          <div className="mb-3">
            <Alert
              ...
              title={("title" in alert) ? alert.title : ""}
              message={("message" in alert) ? alert.message : ""}
              showLink={false}
            />
          </div>
            ...
            {files.map((row: any) => {
              ...
              return (
                <tr key={row._id} className="border-t border-gray-100 dark:border-gray-800">
                  ...
                  {/* Action cell */}
                  <td className="px-6 py-[12px] text-center">
                    <div className="inline-flex items-center gap-1.5">
                      {/* Download */}
                      <IconBtn title="Download" onClick={() => onDownload(row._id, row.name, row.mime)}><DownloadSvg /></IconBtn>

                      {/* Edit / Save / Cancel */}
                      {isEditing ? (
                        <>
                          <IconBtn title="Save" onClick={() => saveEdit(row)}><SaveSvg /></IconBtn>
                          <IconBtn title="Cancel" onClick={cancelEdit}><CancelSvg /></IconBtn>
                        </>
                      ) : (
                        <IconBtn title="Rename" onClick={() => startEdit(row)}><EditSvg /></IconBtn>
                      )}
                      ...
                      <IconBtn
                        ...
                        disabled={!!confirmDeleteId && confirmDeleteId !== row._id}
                      >
                        <TrashSvg />
                      </IconBtn>
                    </div>
