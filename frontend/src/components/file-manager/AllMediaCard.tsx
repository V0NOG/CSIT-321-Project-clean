...
import { listConnectors, StorageConnector } from "../../api/connectorsApi";
...
export default function AllMediaCard() {
  ...
  async function onUploadClick() {
    ...
    try {
      ...
      await encryptWrapAndUpload(file, selectedConnector || undefined);
    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        for (let i = 0; i < acceptedFiles.length; i++) {
          await encryptWrapAndUpload(acceptedFiles[i], selectedConnector || undefined);
    ...
    <div className="relative rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]" {...getRootProps()}>
      ...
      <div className="px-4 py-4 sm:pl-6 sm:pr-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Upload Files</h3>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            {connectors.length > 0 && (
              ...
              >
                <option value="">Default Storage (Dropbox)</option>
                {connectors.map((c) => (
        ...
        <p className="mt-2 text-xs text-gray-400 dark:text-gray-500">
          You can also drag and drop files anywhere on this page. All files are encrypted in your browser before upload.
        </p>
      </div>
    </div>
  );
